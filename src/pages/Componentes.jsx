...
export default function Componentes() {
  ...
  const dados = { massas, recheios, coberturas };
  ...
  return (
    <div>
      ...
      <div className="px-5">
        <Button className="w-full mb-4" onClick={() => navigate(`/componentes/${tab}/novo`)}>
        ...
        {!lista.length ? (
          ...
          <div className="space-y-3 pb-4">
            {lista.map((item) => {
              const totals = calcComponentTotals(item);
              return (
                <Card
                  key={item.id}
                  className="flex items-center gap-3"
                  onClick={() => navigate(`/componentes/${tab}/${item.id}`)}
                >
